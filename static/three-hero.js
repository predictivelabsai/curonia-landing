// Curonia Capital hero — wireframe globe with European/Baltic city dots + flowing arcs.
// Blue palette on white. Slow, restrained. Capped at 30fps. Respects prefers-reduced-motion.

import * as THREE from 'https://unpkg.com/three@0.160.0/build/three.module.js';

const BLUE = 0x2B6CB0;
const BLUE_DIM = 0x1A4971;
...
function init() {
  ...
  const wireMat = new THREE.LineBasicMaterial({ color: INK_DIM, transparent: true, opacity: 0.18 });
  ...
  const fillMat = new THREE.MeshBasicMaterial({ color: 0xDBE9F4, transparent: true, opacity: 0.85 });
  ...
  const dotMat = new THREE.PointsMaterial({ color: BLUE, size: 0.035, transparent: true, opacity: 0.9 });
  globe.add(new THREE.Points(dotGeom, dotMat));

  // City halos (pulsing)
  const haloMat = new THREE.PointsMaterial({ color: BLUE, size: 0.09, transparent: true, opacity: 0.15 });
  ...
  ARCS.forEach(([i, j]) => {
    ...
    const mat = new THREE.LineBasicMaterial({ color: BLUE_DIM, transparent: true, opacity: 0.4 });
  ...
  const pulseMat = new THREE.PointsMaterial({ color: BLUE, size: 0.05, transparent: true, opacity: 0.95 });
